---
title: "Analise da mortalidade por câncer de pulmão"
author: "Agatha Dias"
format:
  dashboard:
     theme: 
...
selectInput(
  ...
  choices = list(
    ...
    "Municípios" = as.list(setNames(
      unique(Cancer_pulmao$local[!Cancer_pulmao$local %in% c(ESTADOS, REGIOES, "Brasil")]),
      unique(Cancer_pulmao$local[!Cancer_pulmao$local %in% c(ESTADOS, REGIOES, "Brasil")])
    ))
  ),
...
output$boxplot <- renderPlot({
  ...
  taxa_max <- max(df_plot$taxa, na.rm = TRUE)
   ...
   ggplot(df_plot, aes(x = local, y = taxa, color = local)) +
...
output$serie <- plotly::renderPlotly({
  ...
  p <- ggplot(df_plot, aes(x = ano, y = taxa, color = local, group = local)) +
    ...
    geom_vline(
      ...
      color = "#4472C4", linewidth = 0.8, linetype = "dashed", alpha = 0.7
    ) +
    annotate("text",
      x     = input$ano, y = Inf,
      label = input$ano,
...
output$vbox_obitos_rj <- renderUI({
  total <- Cancer_pulmao |>
    filter(local == input$muni, ano == input$ano,
           faixa == "todas as idades", sexo == "Ambos") |>
    pull(n)

  div(style = card_style("#919aa1"),
...
output$mapa_rj <- renderGirafe({
  ...
  plot_mapa_rj <- ggplot() +
    ...
    labs(title = glue::glue("Rio de Janeiro · {input$ano}")) +
    theme_void(base_size = 11) +
    theme(
      plot.title.position  = "plot",
      plot.title           = element_text(face = "bold", size = 10,
...
output$serie_rj <- renderPlot({
  ...
  ggplot(df_serie_rj, aes(x = ano, y = taxa)) +
    ...
    geom_vline(
      xintercept = input$ano,
      color = "#1f9bcf", linewidth = 0.8, linetype = "dashed", alpha = 0.7
    ) +
    annotate("text",
      x     = input$ano, y = Inf,
      label = input$ano,
      vjust = 2.5, size = 3,
      color = "#1f9bcf", fontface = "bold"
    ) +
    geom_label(
      data = df_serie_rj |> filter(ano == input$ano),
      aes(
        x = ano,
        y = taxa,
